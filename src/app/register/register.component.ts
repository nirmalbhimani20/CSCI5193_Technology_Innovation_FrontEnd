...
export class RegisterComponent implements OnInit {
  ...
  let test = regexpEmail.test(this.lEmail);
    ...
    if (this.lEmail == null || this.lEmail == "" || this.lEmail == undefined) {
      ...
      this.toster.error("Please insert  valid Email Address", "Error");
    } 
    else if (this.lPassword == null || this.lPassword == "" || this.lPassword == undefined) {
      this.toster.error("Please insert Password", "Error");
    }
    else if (this.lPassword.length < 7) {
      this.toster.error("Minimum 8 Characteres need for password", "Error");
    }
    else {
      var JSON = {
        email: this.lEmail,
  ...
  register() {
    ...
    let regexpEmail =
      new RegExp('^[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,4}$');
    let test = regexpEmail.test(this.rEmail)
    ...
    else if (this.rPassword.length < 7) {
      this.toster.error("Minimum 8 Characteres need for password", "Error");
    }
    else if (this.rPassword != this.rcPassword) {
      this.toster.error("Password And confirm Password is not same", "Error");
    }
